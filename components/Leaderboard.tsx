// Creating the Leaderboard component to display high scores.
          ...
          {scores.length > 0 ? (
            scores.map((score, index) => (
              <div key={index} className="flex justify-between items-center bg-white/10 p-3 rounded-lg text-lg">
                ...
                <span className="flex-grow text-left ml-4">{score.name || 'Anonymous'}</span>
                <span className="font-bold text-yellow-400">{score.score}</span>
                <span className="w-20 text-right uppercase text-xs font-semibold opacity-70">{score.difficulty}</span>
              </div>
            ))
          ) : (
